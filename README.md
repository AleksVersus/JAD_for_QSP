...
3. Перед конвертированием готовых файлов "`.txt`" формата TXT2GAM в "`.qsp`", если включена обработка препроцессором, такие файлы будут дополнительно обработаны встроенным в билдер препроцессором. Так же после обработки препроцессором, непосредственно перед конвертированием, будут запущены python-скрипты, пути к которым прописываются в специльном параметре ["preprocessor"](#preprocessor).
