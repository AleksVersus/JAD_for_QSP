...
0. Скачайте rar-архив из последнего релиза и распакуйте в отдельную папку. В распакованном виде JAD включает в себя папку "`[disdocs]`, и её содержимое, и файл "`QSP.sublime-package.exe`".
...
2. Запустите файл "`QSP.sublime-package.exe`" и следуйте инструкциям — пакет установится автоматически.

### QSP-Builder для Sublime Text

QSP-Builder для Sublime Text (далее QBST, билдер) — это встроенный в пакет QSP плагин, который:
...
Возникающие ошибки выводятся в консоль. Консоль открывается сочетанием клавиш `Ctrl + ~`.
...
* Файл "`Indentation Rules.tmPreferences`" отвечает за поведение отступов в многострочных конструкциях. Автоматически добавляются отступы на новой строке, и автоматически сокращаются перед ключевым словом `end`.
