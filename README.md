...
0. Данная инструкция ориентирована на пользователей, которые скачали последний релиз со страницы релизов: [JAD_for_QSP/releases](https://github.com/AleksVersus/JAD_for_QSP/releases). Скачайте rar-архив из последнего релиза и распакуйте в отдельную папку. В распакованном виде JAD включает в себя папку "`[disdocs]`, и её содержимое, "`QBST`", и её содержимое, и файл "`QSP.sublime-package`", который по сути представляет собой обычный zip-архив с изменённым расширением.
...
Обязательно нужно указать элементы "project", "start", "converter" и "player". Остальные элементы указывать не обязательно.
...
Этот элемент должен содержать массив с перечисленными в нём путями к pyhton-скриптам. Каждый скрипт из перечисленных в этом элементе будет запускаться по одному разу на каждый промежуточный файл в формате TXT2GAM проекта до непосредственного конвертирования этого файла в файл "`.qsp`". При этом, если такой параметр указан для отдельного "билда" проекта, то приоритет имеет параметр, указанный для "билда" (см. раздел [project](#project)).

Каждому файлу скрипта в момент его выполнения в качестве аргумента передаётся путь к промежуточному файлу "билда" в формате TXT2GAM, таким образом вы можете получить доступ к этому файлу прямо из запущенного скрипта, используя переменную `sys.argv[1]`:

```python
import sys

with open(sys.argv[1],'r',encoding="utf-16-le") as file:
	file_string_list=file.readlines()
```

Обратите внимание, промежуточные файлы записаны в кодировке "utf-16-le", поэтому открывать и сохранять данные файлы тоже нужно в кодировке "utf-16-le".
...
Элемент "location" нужен, чтобы указать название локации, которое вы хотите использовать в своей игре. По-муолчанию локация будет называться "prvFile".
...
Элемент "folders" содержит непосредственно пути к папка (в виде массива), из которых выбирается список всех файлов, наличие которых требуется проверять.
...
3. Перед конвертированием готовых файлов "`.txt`" формата TXT2GAM в "`.qsp`", если включена обработка препроцессором, такие файлы будут дополнительно обработаны встроенным в билдер препроцессором. Так же после обработки препроцессором, непосредственно перед конвертированием, будут запущены python-скрипты, пути к которым прописываются в специльном параметре ["postprocessors"](#postprocessors).

##### Урощённый запуск

Упрощённый запуск — это запуск игры без "`project.json`", а вернее "`project.json`" будет создан автоматически. Для такого запуска необходимо, чтобы плеер лежал по адресу "C:\Program Files\QSP\qsp570\qspgui.exe", а утилита txt2gam по адресу "C:\Program Files\QSP\converter\txt2gam.exe", либо нужно вручную отредактировать пути к этим файлам непосредственно в скрипте билдера "`main.py`".
...
* **Конвертер-разделитель** (converter-splitter) — скрипт, который позволяет сконвертировать QSP-файл в файл формата TXT2GAM (qsps), а затем на основе файла QSP-проекта "`.qproj`" разбить получившийся файл на отдельные файлы, каждый из которых содержит отдельную локацию. При этом все файлы размещаются в папках, как указано в файле QSP-проекта, т.е. размещение аналогично размещению по папкам в Quest Generator.

* **Конвертер qps_to_qsps** — скрипт, который составляет часть конвертера-разделителя, но который можно использовать и самостоятельно. Вы можете скормить скрипту QSP-файл и получить на выходе валидный qsps-файл (TXT2GAM).
