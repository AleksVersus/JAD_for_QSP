...
К QBST прилагаются так же файлы "`QSP.sublime-build`" и "`Default.sublime-keymap`", необходимые для установки и корректной работы QBST.
...
4. Следующее, что нужно сделать, это собрать пакет, который будет включать систему сборки QSP-файлов, подсветку синтаксиса и другие элементы для работы с файлами QSP в Sublime Text. Система сборки уже создана, это файл "`QSP.sublime-build`" ("`QSP.sublime-package/QSP.sublime-build`"). Его надо слегка отредактировать:
...
5. Можете так же изменить горячие клавиши для сборки и запуска файлов "`.qsp`". Делается это следующим образом:
	* Откройте файл "`Default.sublime-keymap`" с помощью Sublime Text.
	* Если вам не нравятся назначенные мной сочетания клавиш, перепишите, заменив `["ctrl+f5"]`, `["ctrl+alt+q"]` и `["ctrl+alt+s"]` на иные сочетания.
	* После редактирования сохраните файл "`Default.sublime-keymap`".
6. Теперь упакуйте содержимое папки "`QSP.sublime-package`" в архив zip и измените расширение так, чтобы полное имя архива было "`QSP.sublime-package`".
7. Осталось только установить пакет QSP в ваш Sublime Text. Для этого просто скопируйте созданный вами файл "`QSP.sublime-package`" из папки "`QSP.sublime-package`" в папку "`Packages`" в месте установки вашего Sublime Text. Например, у меня эта папка лежит по адресу "`C:\Program Files\Sublime Text\Packages`".

Примечание: если вы скачивали готовый релиз JAD_QBST начиная с версии 0.4, то в архиве должен лежать готовый пакет QSP. В этом случае нужно извлечь файлы "`Default.sublime-keymap`" и "`QSP.sublime-build`" из пакета (открывается любым архиватором, поддерживающим zip), отредактировать, как указано в пунктах выше, а затем упаковать обратно в пакет. Пакет копируем в папку пакетов Sublime Text, как в пункте 7.
...
3. После редактирования и сохранения файлов можно выбрать один из режимов сборки:
	* "**QSP - qsp-build**" — собрать файлы согласно инструкции и сконвертировать в "`.qsp`", сочетание клавиш `["ctrl+alt+q"]`.
	* "**QSP - qsp-run**" — запустить стартовый файл, указанный в проекте "`project.json`", в плеере. Сочетание клавиш `["ctrl+alt+s"]`.
	* "**QSP**" — собрать файлы согласно инструкции и запустить стартовый файл в плеере, сочетание клавиш `["ctrl+f5"]`.
...
Для того, чтобы в ваших файлах формата TXT2GAM с расширениями "`.qsps`", "`.qsp-txt`", или "`.txt-qsp`", автоматически подсвечивался код QSP, необходимо установить пакет QSP в ваш Sublime Text. Как это сделать, описано в пунктах с 4 по 7 [инструкции по установке QBST](#Установка-QBST).

"`QSP.sublime-package`" обязательно есть в архиве с релизной версией QBST. Если посмотрите исходники проекта, увидите, что в папке "`QSP.sublime-package`" лежит файл "`qsp.sublime-syntax`" — это непосредственно сам файл подсветки, но если его просто скопировать в папку "`Packages`", подсветка работать не будет.

Сам файл "`QSP.sublime-package`" представляет собой zip-архив с изменённым расширением. Именно из него подхватывается подсветка QSP-кода. Данный файл вы можете найти в релизном архиве: https://github.com/AleksVersus/JAD_for_QSP/releases
...
Рядом с подсветкой кода теперь лежат и сниппеты.
