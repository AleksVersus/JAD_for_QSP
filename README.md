...
"`QSP.sublime-package`" обязательно есть в архиве с релизной версией QBST. Если посмотрите исходники проекта, увидите, что в папке "`Syntax-Light`" лежит файл "`qsp.sublime-syntax`" — это непосредственно сам файл подсветки, но если вы скопируете его в папку "`Packages`", подсветка работать не будет.
